...
export default function LoginModal({ isOpen, onClose, onGuestContinue }: LoginModalProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
        ...
        alert(isZh ? '登入失敗，請重試' : 'Sign in failed, please try again');
      } finally {
        setIsLoading(false);
        onClose();
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="relative w-full max-w-md bg-white rounded-xl shadow-xl p-6">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 hover:bg-brand/10 rounded-full transition-colors text-brand-muted"
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 pt-4">
            <button
              ...
              disabled={isLoading}
              className="w-full py-3 px-4 bg-white border-2 border-gray-300 rounded-lg font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center space-x-2"
            >
